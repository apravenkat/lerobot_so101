...
class MotionPlanner:
    ...
    def  inverse_kinematics(self, target_pos, sit_name="gripperframe", max_iter=200, tol=1e-3, step_size=0.05):
        ...
        for _ in range(max_iter):
            ...
            lambda_ = 0.001
            dq = step_size * J_pos.T @ np.linalg.solve(J_pos @ J_pos.T + lambda_ * np.eye(3), error)
    ...
    def plan_motion(self, target_pos, site_name="gripperframe", plan_time=2.0, interpolate_points=50):
        start_q = self.data.qpos.copy()
        goal_q, ik_convereged = self.inverse_kinematics(target_pos, sit_name=site_name)
    ...
    def visualize_trajectory(self, traj, timestep=0.05):
        with mujoco.viewer.launch_passive(self.model, self.data) as viewer:
            for qpos in traj:
                smoothed_qpos = planner.smooth_joint_positions((qpos[:len(planner.joint_names)]))
    ...
    def sendSimPoseToRobot(self, joint_pos):
        ...
        try:
            ...
            print("Sent smoothed joint angles to robot:", sent_action)
        except Exception as e:
